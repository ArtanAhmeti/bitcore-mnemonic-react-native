...
var ExpoRandom = require('expo-random');
...
Mnemonic.randomPhrase = async function(data, wordlist) {
  // if (!(this instanceof Mnemonic)) {
  //     return new Mnemonic(data, wordlist);
  // }
  ...
  phrase = phrase || await Mnemonic._mnemonic(ent, wordlist);
 ...
 * @returns {Buffer}
 */
Mnemonic.phraseToSeed = function(phrase, passphrase) {
  passphrase = passphrase || '';
  return pbkdf2(unorm.nfkd(phrase), unorm.nfkd('mnemonic' + passphrase), 2048, 64);
};
 ...
 */
Mnemonic._mnemonic = async function(ENT, wordlist) {
    var buf = await ExpoRandom.getRandomBytesAsync(ENT / 8);
    // var buf = Random.getRandomBuffer(ENT / 8);
    // var buf = await window.randomBytes(ENT / 8)
